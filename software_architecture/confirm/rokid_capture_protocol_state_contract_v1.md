...
这些项目进入第 21 节的 M0 真机验收，不得在业务层写死。
...
| `boot_id` | 眼镜每次开机或采集进程全新启动 | 眼镜 | 区分单调时钟和序号重置 |
| `stream_id` | 每次从 A0 进入 A1 或重新启动音频采集 | 眼镜 | A1/A2 共用的连续音频流身份 |
...
作为原始身份，`conversation_id` 必须允许为空。进入 A2 时，手机写入不可变的 `ConversationBindingV1`：

```json
{
  "schema": "memory.conversation-binding.v1",
  "conversation_id": "0191...",
  "stream_id": "0191...",
  "boot_id": "0191...",
  "first_source_seq": 88012,
  "first_voice_source_seq": 88112,
  "bound_through_source_seq": 88420,
  "preroll_ms": 2000,
  "created_phone_monotonic_ms": 18531000
}
```

后续 A2 帧可携带 `conversation_id` 作为冗余字段，但其真实归属仍以 `ConversationBindingV1` 和 `source_seq` 范围为准。
...
1. `control_epoch` 小于已保存值：拒绝，返回 `STALE_EPOCH`。
2. `control_epoch` 相同且 `command_seq` 小于已保存值：拒绝，返回 `STALE_COMMAND`。
3. `command_id` 已处理：返回原 ACK，不重复执行副作用。
4. `control_epoch` 更大：进入重同步，先使所有旧租约失效，再应用命令。
5. `ENTER_PRIVATE` 必须先持久化新 epoch，再停止采集；崩溃恢复后不得回到旧租约。
6. 普通命令的 `ttl_ms` 已过期时不得执行。
...
| `START_LISTENING` | A0→A1，签发音频租约 | codec、lease、spool 上限 |
| `BIND_CONVERSATION` | 将 source_seq 范围绑定 A2 | `ConversationBindingV1` |
| `ENTER_END_PENDING` | 进入会话结束候选 | 静音、上下文原因 |
| `CANCEL_END_PENDING` | 人声恢复，回 A2 active | conversation_id |
| `FINALIZE_CONVERSATION` | 封存会话 | end_reason、最后期望序号 |
...
| `ACK_SPOOL_RANGE` | 确认手机已持久化补偿帧 | stream_id、committed_through_seq |
...
5. 手机根据 `last_phone_committed_capture` 和 `last_phone_committed_source_seq` 请求缺失数据。
...
| 当前状态 | 事件 | 下一状态 | 必须动作 |
|---|---|---|---|
| A0 | 用户恢复且隐私允许 | A1 | 新建 stream 或重新启动；签发音频租约 |
| A1 | 持续人声/日程/手动开始 | A2_ACTIVE | 创建 conversation、冻结预录、写 binding |
| A1 | 隐私禁止 | A0 | 清空未绑定预录和 spool，停止采集 |
| A2_ACTIVE | 静音+上下文候选 | A2_END_PENDING | 继续录音，不关闭 chunk |
| A2_END_PENDING | 人声恢复 | A2_ACTIVE | 取消结束候选 |
| A2_ACTIVE/END_PENDING | 正常/手动结束 | A2_FINALIZING | 关闭最终分片、校验、提交 final manifest |
| A2_ACTIVE/END_PENDING | 隐私禁止 | A0 | 先本地封存旧会话，再停止；异步 finalize |
...
- 实时音频数据通道断开但租约有效时，眼镜进入 `DISCONNECTED_SPOOLING`。
- spool 达到时间或存储上限后，丢弃最旧未绑定 A1 帧；A2 帧不得静默丢弃，必须生成 gap 事件并停止或按策略降级。
- 手机在眼镜音频链路不可用时启动手机麦克风兜底，并写 `source_history`。
...
眼镜将 spool 按 5 秒左右的不可变 block 保存：
...
手机只有在补偿帧和元数据事务提交后，才发送 `ACK_SPOOL_RANGE`。眼镜收到 ACK 后记录 range tombstone，再删除被完整覆盖的 block。
...
1. 持久化新 `control_epoch` 和 `PRIVACY_ENTERED` 事件。
2. 使全部音频/视觉租约失效。
3. 停止音频回调和实时发送。
4. 停止视觉 timer，调用相机关闭。
...
若眼镜端按键/语音提供本地暂停，它必须先执行本地 A0/L0，再向手机发送事件。手机不得因为自身状态仍是 A1/A2 而自动反向恢复；只有用户明确恢复后才能签发新 epoch 和新租约。
...
```json
{
  ...
  "source_history": [
    {"source": "GLASSES", "start_timeline_ms": 0, "end_timeline_ms": 602000}
  ],
  "sealed_phone_monotonic_ms": 19134000
}
```

`expected_sequences` 不再只表示 chunk 编号；云端必须同时校验基础 chunk、repair 和 source_seq 时间线覆盖。
...
| 实时数据通道断开 | 蓝牙控制保留；音频 spool；手机麦克风兜底 |
...
2. 先发送 L3，再发送更高 epoch 的 A0，随后重放旧 L3；相机保持关闭并返回 `STALE_EPOCH`。
3. L1/L3 租约过期且手机失联，眼镜在规定时间内自动 L0。
4. 眼镜进程在 A0 各持久化点被强杀，重启后均不能恢复采集。

### 29.2 音频

1. A1 帧 `conversation_id=null`，进入 A2 后通过 binding 正确固化前 2 秒。
2. 人为丢弃 1%、5%、20% 实时帧，source_seq 缺口可检测。
3. 实时音频通道中断 30/60/120 秒，spool 补传去重后无重复样本。
4. 补偿帧晚于基础 chunk 上传到达，生成 repair version，不修改旧 SHA。
5. 眼镜重启后 boot_id 变化，序号重置不与旧流冲突。
6. 切手机麦克风时保留 1-2 秒重叠并写 source history。
...
1. 建立 `protocol` 模块：ID、schema、枚举、错误码、序列化和签名。
2. 实现双端持久化 reducer、epoch/command_seq、状态快照和 A0 抢占。
3. 用 Fake adapter 跑通命令去重、L1/L3 租约和 ACK1 崩溃测试。
4. 接入 Rokid 双端 SDK，完成 R0-01 至 R0-09。
5. 实现 A1 流、conversation binding、手机预录和 60 秒 chunk。
6. 实现眼镜 spool、range ACK、不可变 repair。
7. 实现视觉 journaled bundle、P2P batch 和双 ACK。
8. 接入云端幂等 Ingestion、完整性屏障和版本化 ASR/OCR。
9. 完成故障注入、隐私、安全和续航验收后冻结 v1.0。
...
| A1 帧身份 | stream_id + boot_id + source_seq；conversation_id 可空 |
| 会话绑定 | 手机进入 A2 时用 source_seq range 固化预录和归属 |
| 视觉调度 | 手机签发租约，眼镜本地 timer，不逐张远程调用 |
| 隐私防乱序 | control_epoch + command_seq fencing；A0 增加 epoch |
